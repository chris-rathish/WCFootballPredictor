...
declare
  ...
  g_sf     int;
begin
  -- actual advancers so far (winners of each completed round)
  ...
  -- games per source round (a stage is fully correct when every advancer matches)
  ...
  -- Scoring: 20 points per stage predicted FULLY correctly (all advancers), else 0.
  -- correct = g means the round is complete AND every advancer was predicted.
  for b in select * from public.brackets loop
    ...
    -- final + third place = one stage: both must be correct
